...
// 获取当前页面实例，pages数组最后一项
const pageInstance = pages.at(-1) as any // uniapp没有提供页面实例的类型声明

// 页面渲染完毕，绑定导航栏滚动动画效果
onReady(() => {
  // 原生小程序组件实例上添加动画效果
  // 滚动驱动动画: https://developers.weixin.qq.com/miniprogram/dev/framework/view/animation.html#%E6%BB%9A%E5%8A%A8%E9%A9%B1%E5%8A%A8%E7%9A%84%E5%8A%A8%E7%94%BB
  const scrollTimeline = {
    scrollSource: '#scroller', // 指定滚动元素的选择器
    timeRange: 1000, // 起始和结束的滚动范围映射的时间长度
    startScrollOffset: 0, // 指定开始驱动动画进度的滚动偏移量
    endScrollOffset: 50, // 指定停止驱动动画进度的滚动偏移量
  }
...
const onOrderPay = async () => {
  if (isDev) {
    // 开发环境模拟支付
    const { result } = await getPayMockAPI({ orderId: query.id })
  } else {
    // #ifdef MP-WEIXIN
    // 正式环境微信支付
    const { result } = await getPayWxPayMiniPayAPI({ orderId: query.id }) // 获取支付参数
    wx.requestPayment(result) // 调用微信支付
    // #endif
  }
...
const onOrderDelete = () => {
  uni.showModal({
    ...
    success: async (success) => {
      if (success.confirm) {
        const { result } = await deleteMemberOrderAPI({ ids: [query.id] })
...
<template>
  <!-- 自定义导航栏: 默认背景透明, scroll-view 滚动到 50 时展示 -->
